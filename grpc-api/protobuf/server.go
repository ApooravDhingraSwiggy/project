...
func toUser(data userDetails) *User {
	return &User{
		Uid:         data.Uid,
		Name:        data.Name,
...
func fromUser(user *User) userDetails {
	return userDetails{
		Uid:         user.GetUid(),
		Name:        user.GetName(),
...
func (s *server) FetchUser(ctx context.Context, req *FetchUserRequest) (*FetchUserResponse, error) {
